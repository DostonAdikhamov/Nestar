...
export class PropertyService {
    ...
    private shapeMatchQuery(match: T, input: PropertiesInquiry): void {
      ...
      if(periodsRange) match.propertyPrice = { $gte: periodsRange.start, $lte: periodsRange.end};
      if(squaresRange) match.propertyPrice = { $gte: squaresRange.start, $lte: squaresRange.end};
    ...
    public async getAgentProperties(memberId: ObjectId, input: AgentPropertiesInquiry): Promise<Properties> {
      ...
      const match: T = {
        memberId: memberId,
        propertyStatus: propertyStatus ?? { $in: PropertyStatus.DELETE}
      };
      const sort: T = { [input?.sort ?? "createdAt"]: input?.direction ?? Direction.DESC}
